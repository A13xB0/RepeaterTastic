...
func TestSetupMeshtasticNode(t *testing.T) {
	srv := testWeb(t)
	probe := func(dev string) (int, map[string]any) {
		code, obj, _ := call(t, srv, "POST", "/api/v1/setup/probe", "", map[string]any{"driver": "meshtastic", "device": dev})
		return code, obj
	}
	// Before a password exists the probe stays on this machine and the LAN.
	if code, obj := probe("203.0.113.5"); code != 400 || !strings.Contains(obj["error"].(string), "local network") {
		t.Fatalf("public address: %d %v", code, obj)
	}
	for _, dev := range []string{"", "a:b:c:d", "/etc/passwd"} {
		if code, obj := probe(dev); code != 400 {
			t.Fatalf("%q: %d %v", dev, code, obj)
		}
	}
	if code, obj := probe("127.0.0.1:1"); code != 200 || obj["ok"] != false || !strings.Contains(obj["error"].(string), "no Meshtastic node answered") {
		t.Fatalf("nothing listening: %d %v", code, obj)
	}
	code, obj, _ := call(t, srv, "POST", "/api/v1/setup", "", map[string]any{"password": "correct horse", "driver": "meshtastic"})
	if code != 400 || !strings.Contains(obj["error"].(string), "meshtasticd address") {
		t.Fatalf("setup without a device: %d %v", code, obj)
	}
}

func TestLANAddr(t *testing.T) {
	for addr, want := range map[string]bool{
		"127.0.0.1:4403": true, "192.168.1.20:4403": true, "[fe80::1]:4403": true, "10.10.98.223:4403": true,
		"203.0.113.5:4403": false, "[2001:db8::1]:4403": false, "nonsense": false,
	} {
		if got := lanAddr(context.Background(), addr); got != want {
			t.Errorf("%s: %v", addr, got)
		}
	}
}

func TestSwitchToNodeNeedsRestart(t *testing.T) {
	s := &Server{cfg: config.Default(), booted: config.Default()}
	s.cfg.Radio = config.Radio{Driver: "meshtastic", Device: "127.0.0.1:4410"}
	if r := s.restartReasons(); len(r) == 0 {
		t.Fatal("switching a modem to a Meshtastic node needs a restart")
	}
}
