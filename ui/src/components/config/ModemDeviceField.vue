...
// Where a radio is: a USB serial port running the KISS firmware (driver kiss), a LoRa HAT or USB
// stick the spi driver runs itself (driver spi, device a board), or a node running Meshtastic
// firmware reached over USB or the network (driver meshtastic, device a port or host[:port]).
  ...
  <div>
    <div class="flex flex-wrap items-center justify-between gap-2">
      ...
      <div class="seg" role="group" :aria-label="`${label} connection`">
        ...
        <button type="button" :aria-pressed="kind === 'board'" @click="kind = 'board'">HAT or stick</button>
        <button type="button" :aria-pressed="kind === 'node'" @click="kind = 'node'">Meshtastic node</button>
      </div>
    </div>
    ...
    <template v-if="kind === 'serial'">
      ...
      <p class="hint">
        A board flashed with the Mesh KISS firmware. RepeaterTastic runs the mesh and can host many identities on it. Prefer
        <span class="mono">/dev/serial/by-id/…</span> so the path survives replugging.<template v-if="restartHint"> Changing it needs a restart.</template>
      </p>
    </template>
    ...
    <template v-else-if="kind === 'board'">
      ...
      <p class="hint">
        A LoRa HAT on SPI or a CH341 USB stick that RepeaterTastic drives itself (experimental), with no meshtasticd on it.<template v-if="restartHint"> Changing it needs a restart.</template>
      </p>
    </template>
    ...
    <template v-else>
      <div class="mt-2 flex flex-wrap items-center gap-2">
        ...
        <input v-if="via === 'usb'" :id="`${props.id}-node-serial`" v-model="nodePort" class="input mono min-w-0 flex-1" :list="`${props.id}-node-ports`" placeholder="/dev/ttyACM0" aria-label="Node serial port" spellcheck="false" />
        <template v-else>
          <input :id="`${props.id}-node-host`" v-model="host" class="input mono min-w-0 flex-1" placeholder="192.168.1.20 or meshtastic.local" aria-label="Node address" spellcheck="false" />
          <input :id="`${props.id}-node-port`" v-model.number="port" type="number" min="1" max="65535" class="input mono w-24" aria-label="Node API port" />
        </template>
      </div>
      ...
      <p class="hint">
        A board running stock Meshtastic firmware (Heltec, T-Beam, RAK, T-Echo…) or a meshtasticd. The node does the radio work and is this radio's one identity; region, preset and role are written to it.
