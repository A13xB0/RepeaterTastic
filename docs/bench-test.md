...
The installer also sets up meshtasticd 2.8+ (`--meshtasticd auto` by default), which every node
runs on: without it nothing goes on air. Open `http://<host>:8080`, set the admin password, and
